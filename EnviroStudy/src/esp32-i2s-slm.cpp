...
#define DEBUG_AUDIO       1           // Set to 0 to disable NaN diagnostics
...
SOS_IIR_Filter DC_BLOCKER(
  sizeof(DC_BLOCKER_sos) / sizeof(DC_BLOCKER_sos[0]),
  1.0,
  DC_BLOCKER_sos
);
...
SOS_Coefficients ICS43432_sos[] = { // Second-Order Sections {b1, b2, -a1, -a2}
...
SOS_IIR_Filter ICS43432(
  sizeof(ICS43432_sos) / sizeof(ICS43432_sos[0]),
  -0.457337023383413,
  ICS43432_sos
);
...
SOS_Coefficients INMP441_sos[] = { // Second-Order Sections {b1, b2, -a1, -a2}
  {-1.986920458344451, +0.986963226946616, +1.995178510504166, -0.995184322194091}
};
SOS_IIR_Filter INMP441(
  sizeof(INMP441_sos) / sizeof(INMP441_sos[0]),
  1.00197834654696,
  INMP441_sos
);
...
SOS_IIR_Filter IM69D130(
  sizeof(IM69D130_sos) / sizeof(IM69D130_sos[0]),
  1.00124068496753,
  IM69D130_sos
);
...
// With additional DC blocking component
SOS_Coefficients SPH0645LM4H_B_RB_sos[] = { // Second-Order Sections {b1, b2, -a1, -a2}
  {-1.0, 0.0, +0.9992, 0}, // DC blocker, a1 = -0.9992
  {-1.988897663539382, +0.988928479008099, +1.993853376183491, -0.993862821429572}
};
SOS_IIR_Filter SPH0645LM4H_B_RB(
  sizeof(SPH0645LM4H_B_RB_sos) / sizeof(SPH0645LM4H_B_RB_sos[0]),
  1.00123377961525,
  SPH0645LM4H_B_RB_sos
);
...
SOS_IIR_Filter C_weighting(
  sizeof(C_weighting_sos) / sizeof(C_weighting_sos[0]),
  -0.491647169337140,
  C_weighting_sos
);
...
void setup() {
  ...
  while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    #if (DEBUG_AUDIO > 0)
    if (!isfinite(q.sum_sqr_SPL) || !isfinite(q.sum_sqr_weighted)) {
      Serial.printf("[DBG] non-finite filter output block=%lu SPL=%f weighted=%f ticks=%lu\n",
                    (unsigned long)debug_block_count,
                    q.sum_sqr_SPL,
                    q.sum_sqr_weighted,
                    (unsigned long)q.proc_ticks);
    }
    #endif

    // Calculate dB values relative to MIC_REF_AMPL and adjust for microphone reference
    double short_mean_sqr = double(q.sum_sqr_SPL) / SAMPLES_SHORT;
    double short_RMS = sqrt(short_mean_sqr);
    double short_ratio = short_RMS / MIC_REF_AMPL;
    double short_SPL_dB = MIC_OFFSET_DB + MIC_REF_DB + 20 * log10(short_ratio);

    #if (DEBUG_AUDIO > 0)
    if ((short_mean_sqr < 0) || !isfinite(short_RMS) || !isfinite(short_ratio) || !isfinite(short_SPL_dB)) {
      Serial.printf("[DBG] short path block=%lu mean_sqr=%e RMS=%e ratio=%e MIC_REF_AMPL=%e short_dB=%f\n",
                    (unsigned long)debug_block_count,
                    short_mean_sqr,
                    short_RMS,
                    short_ratio,
    ...
    if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
      double Leq_mean_sqr = Leq_sum_sqr / Leq_samples;
      double Leq_RMS = sqrt(Leq_mean_sqr);
      double Leq_ratio = Leq_RMS / MIC_REF_AMPL;
      Leq_dB = MIC_OFFSET_DB + MIC_REF_DB + 20 * log10(Leq_ratio);

      #if (DEBUG_AUDIO > 0)
      if ((Leq_mean_sqr < 0) || !isfinite(Leq_RMS) || !isfinite(Leq_ratio) || !isfinite(Leq_dB)) {
        Serial.printf("[DBG] Leq path mean_sqr=%e RMS=%e ratio=%e MIC_REF_AMPL=%e Leq_dB=%f\n",
                      Leq_mean_sqr,
                      Leq_RMS,
                      Leq_ratio,
                      MIC_REF_AMPL,
      ...
      Serial.printf(">dBA: %.1f\n", Leq_dB);

      #if (DEBUG_AUDIO > 0)
      Serial.printf("[DBG] last-block sums SPL=%f weighted=%f ticks=%lu\n",
                    q.sum_sqr_SPL,
                    q.sum_sqr_weighted,
                    (unsigned long)q.proc_ticks);
      #endif
